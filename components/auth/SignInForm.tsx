...
export function SignInForm() {
  ...
  const handleSocialLogin = (provider: string) => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      const demoEmail = `collector.${provider.toLowerCase()}@artgallery.com`;
      signIn(demoEmail);
      setIsLoading(false);
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Email Field */}
        <div>
          <label htmlFor="signin-email" className="block text-xs font-medium text-cream/80">
            Email Address
          </label>
        ...
        <div>
          <div className="flex items-center justify-between">
            <label htmlFor="signin-password" className="block text-xs font-medium text-cream/80">
              Password
            </label>
            <button
              type="button"
              onClick={() => setShowForgotModal(true)}
              className="text-xs text-gold/80 transition-colors hover:text-gold hover:underline"
            >
              Forgot password?
            </button>
          </div>
        ...
        <div className="flex items-center pt-1">
          <label className="flex items-center gap-2.5 cursor-pointer text-xs text-cream/70 select-none">
            <input
              type="checkbox"
              checked={rememberMe}
              onChange={(e) => setRememberMe(e.target.checked)}
              className="h-4 w-4 rounded border-white/20 bg-black/50 text-gold focus:ring-gold/30 focus:ring-offset-0 accent-gold cursor-pointer"
            />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-panel p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h3 className="font-display text-lg text-cream">Reset Password</h3>
              <button
                type="button"
            ...
            {resetSent ? (
              <div className="py-6 text-center text-xs text-emerald-300 space-y-2">
                <svg className="mx-auto h-8 w-8 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <form onSubmit={handleForgotPassword} className="mt-4 space-y-4">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/10 bg-black/40 px-3.5 py-2.5 text-xs text-cream placeholder-muted/50 focus:border-gold focus:outline-none"
                />
                <div className="flex justify-end gap-2 pt-2">
                  <button
                    type="button"
                    onClick={() => setShowForgotModal(false)}
                    className="rounded-lg px-3 py-2 text-xs text-muted hover:text-cream"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="rounded-lg bg-gold-btn px-4 py-2 text-xs font-medium text-black hover:bg-gold-light"
                  >
                    Send Link
                  </button>
                </div>
